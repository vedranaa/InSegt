...
class Annotator(PyQt5.QtWidgets.QWidget):
    ...
    def __init__(self,filename):
        ...
        self.annotationPix.fill(color_picker(label=0, opacity=0))
        self.cursorPix = PyQt5.QtGui.QPixmap(self.imagePix.width(),self.imagePix.height())
        self.cursorPix.fill(color_picker(label=0, opacity=0))    
    ...
    def drawCursorPoint(self, point):
        ...
        self.cursorPix.fill(color_picker(label=0, opacity=0)) # transparent
        painter_scribble = self.makePainter(self.cursorPix, 
                    color_picker(self.label, self.opacity)) # the painter used for cursor
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == PyQt5.QtCore.Qt.LeftButton: 
            if self.zPressed: # initiate zooming and not drawing
                self.cursorPix.fill(color_picker(label=0, opacity=0)) # clear (fill with transparent)
                ...
                painter_scribble = self.makePainter(self.annotationPix, 
                        color_picker(self.label, self.annotationOpacity())) # the painter used for drawing        
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.activelyZooming: 
            ...
            painter_scribble.fillRect(x,y,w,h,color_picker(0,0.75))
        else:          
    ...
    def executeZoom(self):
        ...
        print('   Zooming to ' + formatQRect(self.source))     
    ...
    def resetZoom(self):
        ...
        print(f'   Reseting zoom to ' + formatQRect(self.source))        
